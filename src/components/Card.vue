<template>
  <div :key="'recent' + index" class="card flex-auto w-full bg-black bg-opacity-70 relative grid grid-cols-4 grid-rows-1 overflow-hidden transition duration-300 border-b border-gray-600">
    <div class="col-span-1 aspect-ratio-square overflow-hidden relative">
      <transition name="fade-up" mode="out-in" appear>
        <img :key="'cover' + index + updatedCover" ref="coverArt" v-show="hasLoaded" @load="loaded" @error='updatedCover = aurLogo' v-loadedifcomplete crossorigin="anonymous" :src="updatedCover" class="h-full w-full object-cover bg-cover ring-2 ring-purple-200 ring-opacity-25 transition-transform duration-300 absolute" alt="" />
      </transition>
      <transition name="fade-up" mode="out-in" appear>
        <div :key="'skelly' + index + updatedCover" v-show="!hasLoaded" class="box-border h-full w-full bg-gradient-to-br from-gray-700 to-gray-600 bg-opacity-50 grad ring-2 ring-purple-200 ring-opacity-25 transition duration-300 absolute"></div>
      </transition>
    </div>
