<template>
  <div :key="'recent' + index" class="flex-grow-0 w-full rounded-none xxs:rounded-2xl p-2 xxs:p-4 bg-black bg-opacity-60 relative grid grid-cols-4 grid-rows-1 overflow-hidden transition duration-300 border-b border-black-light xxs:border-none">
    <div class="col-span-1 aspect-ratio-square overflow-hidden relative rounded-xl md:rounded-2xl">
      <transition name="fade-up" mode="out-in" appear>
        <img :key="'cover' + index" ref="coverArt" v-show="hasLoaded" @load="loaded" @error='updatedCover = aurLogo' v-loadedifcomplete :src="updatedCover" class="h-auto w-full object-cover ring-2 ring-purple-200 ring-opacity-25 transition-transform duration-300 absolute transform scale-105" alt="" />
      </transition>
      <transition name="fade-up" mode="out-in" appear>
