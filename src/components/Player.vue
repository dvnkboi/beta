            ...
            init: () => {
               try {
                  this.currentSongTimer.time =
                     (Date.now() -
                        new Date(
                           this.res.response.history[0].date_played
                        ).getTime() -
                        this.audioLatency -
                        1000) /
                     1000;
      ...
      importAudio() {
         ...
         if (!this.audio) {
            this.audio = new Silence(
               "https://api.ampupradio.com:8443/TOP40.mp3?nocache=" +
                  Date.now(),
               {
      ...
      async getWikiPage() {
         ...
         try {
            let res = await this.axios.get(
               proxy.wikiPageUrl + processedArtist,
               {
                  responseType: "json",
            ...
            try {
               for (i = startIndex; i < this.covers; i++) {
                  ...
                  this.queue[i].minutes = Math.floor(
                     (new Date().getTime() -
                        new Date(
                           this.res.response.history[i].date_played
                        ).getTime()) /
                        60000
                  );
