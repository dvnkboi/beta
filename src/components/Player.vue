<template>
  <div
    class="w-full flex justify-start items-start flex-col xl:flex-row xl:h-full relative"
  >
    ...
    <SongBg
      ...
      :changed="queue[0].changed"
      :currentColor="currentPalette.Vibrant.hex"
      :percent="currentSongTimer.percent"
      :cover="queue[0].largeCover"
      :normalizedBassData="normalizedBassData"
    />
...
export default {
      ...
      currentSongTimer: {
        timer: new AdjustingInterval(
          () => {
            try {
              ...
              this.currentSongTimer.percent =
                this.currentSongTimer.time /
                this.res.response.history[0].duration;
        ...
        init: () => {
          try {
            this.currentSongTimer.time =
              (Date.now() -
                new Date(this.res.response.history[0].date_played).getTime() -
                this.audioLatency -
                1000) /
              1000;
            if (this.currentSongTimer.timer.running)
              this.currentSongTimer.timer.stop();
  ...
  methods: {
    ...
    importAudio() {
      let proxy = this;
      if (!this.audio) {
        this.audio = new Silence(
          "https://api.ampupradio.com:8443/TOP40.mp3?nocache=" + Date.now(),
          {
        ...
        this.audio.normalDataFn = data => {
          ...
          proxy.normalizedBassData =
            proxy.normalizedBassData < 0.97 ? proxy.normalizedBassData : 1;
        };
    ...
    async queueReqStack() {
      ...
      if (!this.lodashGet) {
        this.lodashGet = await import(
          /* webpackChunkName: "lodash-get" */ "lodash.get"
        );
    ...
    async getWikiPage() {
      ...
      try {
        ...
        let relevancyCheck = Object.values(
        ...
        if (
          ...
          !relevancyCheck.includes("may refer to")
        )
    ...
    async getQueue() {
      if (this.queueOpen && this.connected) {
        ...
        this.res = await Promise.retry(3, this.getHistory, 1000).catch(e =>
          console.log(e.message)
        );
        this.art = this.lodashGet(
          await Promise.retry(3, this.getArt, 1000).catch(e =>
            console.log(e.message)
          ),
          "response"
        );
      ...
      try {
        ...
        if (
          this.previousID.value ==
            this.lodashGet(this.art[this.previousID.index][0], "_id") &&
          !noCheckDup
        ) {
          setTimeout(() => {
            this.queueOpen = true;
        ...
        try {
          for (i = startIndex; i < this.covers; i++) {
            ...
            this.queue[i].minutes = Math.floor(
              (new Date().getTime() -
                new Date(this.res.response.history[i].date_played).getTime()) /
                60000
            );
            ...
            if (this.queue[i].cover.includes("undefined"))
              this.queue[i].cover = this.aurTmpLogo;
            ...
            if (this.queue[i].largeCover.includes("undefined"))
              this.queue[i].largeCover = this.aurTmpLogo;
      ...
      Vibrant.from(this.queue[0].cover)
        ...
        .then(palette => {
          ...
          document.documentElement.style.setProperty(
            "--maskR",
            palette.Vibrant.rgb[0]
          );
          document.documentElement.style.setProperty(
            "--maskG",
            palette.Vibrant.rgb[1]
          );
          document.documentElement.style.setProperty(
            "--maskB",
            palette.Vibrant.rgb[2]
          );
        })
        .catch(() => {
          let nextColor =
            proxy.colors[Math.floor(proxy.colors.length * Math.random())];
          proxy.currentPalette.Vibrant.hex =
            nextColor == proxy.currentPalette.Vibrant.hex
              ? proxy.colors[
                  (proxy.colors.indexOf(proxy.currentPalette.Vibrant.hex) + 1) %
                    proxy.colors.length
                ]
              : nextColor;
        });
    ...
    async preloadNext() {
      ...
      if (src) {
        ...
        preloadImg.src = preloadSrc.includes("undefined")
          ? this.aurTmpLogo
          : preloadSrc;
        this.preloadSuccess = false;
    ...
    async reconnectSocket() {
      ...
      if (!this.io) {
        this.io = await import(
          /* webpackChunkName: "socketIO.client" */ "socket.io-client"
        );
    ...
    fillQueue() {
      ...
      for (var i = 0; i < this.covers - this.queue.length; i++)
        this.queue.push({});
      this.covers = 10;
